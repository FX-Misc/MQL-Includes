...
int start()
  {
   int    i,start_pos,i_time,time0,last_fpos=0,periodseconds;
   double d_open=0,d_low=0,d_high=0,d_close=0,d_volume=0,last_volume=0;
   int    hwnd=0,cnt=0;
//--- History header
   int    version=400;
   string c_copyright;
   ...
   int    i_unused[13];
//---  
   ExtHandle=FileOpenHistory(c_symbol+IntegerToString(i_period)+".hst",FILE_BIN|FILE_WRITE);
   if(ExtHandle<0)
      return(-1);
